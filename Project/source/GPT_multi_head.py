...
class Block(nn.Module):
    ...
    def forward(self, x):
        x = x + self.sa(self.ln1(x))  # Skip connection après Self-Attention
        x = x + self.ffwd(self.ln2(x))  # Skip connection après Feed-Forward
        return x
...
class BigramLanguageModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.blocs = nn.Sequential(*[Block(n_embd, n_head) for _ in range(3)])
